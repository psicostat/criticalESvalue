...
\textcolor{red}{Beyhond critical effect size values, researchers should reason before conducting a study on what conclusions can be deawn, and for those cases where sample size constrains will make it impossible to find an effect bacuse it is likely smaller than the critical effect size value, alternative options for data collection, such as multi-lab studies (\cite{AAA}), should be explored.}
