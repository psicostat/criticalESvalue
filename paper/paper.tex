...
\textcolor{red}{The proposed tool is useful for published studies where a power analysis was not performed. If the power analysis is based on the smallest effect size of interest, the critical effect size does not add information. But if the sample size was purely justified based on resource constraints - as it is often the case - and there is no power analysis - then the critical effect size adds meaningful information, focusing the attention more strongly on which effect sizes are reasonable to expect. Once again, it is not always possible to determine a range of non relevant results, because it would require specifying the smallest effect size of interest, in such cases, we contend that reporting critical effect sizes can help put into perspective and convey the relevance of the results. }
...
\textcolor{blue}{We suggest that reporting critical effect sizes is particularly valuable when sample size planning was not feasible or did not occur \textit{a priori}. In cases where optimal power can be attained with a sufficiently large sample size for an effect of a specific magnitude of interest, and this is truly determined \textit{a priori}, the interpretation of both significance and non-significance becomes straightforward. However, when power analysis did not inform the sample size or when power is likely but undeterminedly low, reporting critical effect sizes for the obtained sample can help provide context for interpretation. Critical effect sizes can be computed and interpreted even retrospectively or for studies that have already been published.}
