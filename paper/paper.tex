...
\abstract{%
Critical effect size(s) represent the smallest detectable effect that would reach statistical significance given a certain sample size \textcolor{red}{and alpha level. We argue that these values are useful to calculate and report to help interpret both statistically significant and non-significant effect sizes in NHST (Null Hypothesis Significance Testing), especially when the sample size could not be carefully planned a priori, or when there is uncertainty about the expected effect size that can be collected. Also, based on the smallest effect that can be significant, researchers may evaluate whether it is desirable that smaller effects can be detected, even without knowing a priori what the expected effect size may be.} To assist researchers in calculating critical effect sizes, we have developed an R package that allows to report the critical effect sizes of model parameters. The package's functions are implemented for group comparisons, correlations, linear regression, and meta-analysis. What we propose could benefit researchers during the planning phase of the study by helping them to understand the limitations and strengths of their research design, and also as a retrospective tool to potentially reframe original interpretations, \textcolor{red}{ especially of non significant results.}
}
...
In psychology, statistical power is often severely limited for medium to small effect sizes (Szucs \& Ioannidis, 2017), \textcolor{red}{which are also the range of the majority of effects in this field}. In fact, resource constraints have been listed among acceptable justifications for sample size (Lakens, 2022), and this scenario is arguably widespread in psychology. However, limited power makes it challenging to distinguish signal from noise, and on average it results in an overestimation of effect sizes when relying on statistical significance for inference (Altoè et al., 2020). \textcolor{red}{Also, the tool we propose is useful for interpreting published studies where a power analysis was not (appropriately) performed. Of course, if a power analysis was conducted and it was based on the smallest effect size of interest, the critical effect size does not add any information, as the study is certainly powered enough to detect as significant even the smallest effect that could be meaningful to the researchers. However, if the sample size was purely justified based on resource constraints - as it is often the case -, then the critical effect size adds meaningful information, providing a benchmark against which assessing what effect sizes are reasonable to expect. Once again, it is not always possible to determine a range of non-relevant results, because it would require specifying the smallest effect size of interest, in such cases, we contend that reporting critical effect sizes can help put into perspective and convey the relevance of the results.}
